...
__global__ void evolve(const Cell* current, Cell* next, int width, int height) {
    ...
    for (int dy = -1; dy <= 1; ++dy)
        for (int dx = -1; dx <= 1; ++dx)
            if (dx || dy) neighbors += current[(y + dy) * width + x + dx];
    int i = y * width + x;
...
public:
    Simulation(const Grid& initial, int w, int h)
        : grid(shape(w, h), MPI_COMM_WORLD), width(w), height(h),
        ...
        for (int y = 1; y < local_h - 1; ++y)
            for (int x = 1; x < local_w - 1; ++x)
                grid.current_data(y, x) = initial[
                    (grid.topology.starts[0] + y - 2) * w +
        ...
        } catch (...) { cudaFree(a); cudaFree(b); throw; }
    }
    Simulation(const Simulation&) = delete;
    Simulation& operator=(const Simulation&) = delete;
    ~Simulation() { cudaFree(a); cudaFree(b); }
    void step() {
        // 按维度交换，第二维会传递第一维的 halo，从而覆盖对角邻居。
        grid.exchange_halos_noneblocking();
        check(cudaMemcpy(a, grid.current_data.data(), grid.current_data.size(), cudaMemcpyHostToDevice));
        dim3 threads(16, 16);
        dim3 blocks((local_w - 2 + 15) / 16, (local_h - 2 + 15) / 16);
        evolve<<<blocks, threads>>>(a, b, local_w, local_h);
        check(cudaGetLastError());
        check(cudaMemcpy(grid.next_data.data(), b, grid.next_data.size(), cudaMemcpyDeviceToHost));
    ...
    Grid read() const {
        ...
        MPI_Allreduce(MPI_IN_PLACE, result.data(), static_cast<int>(result.size()),
                      MPI_UNSIGNED_CHAR, MPI_MAX, grid.topology.comm_cart);
        return result;
    }
    void write(const std::string& path) const {
        // 上游写入器不截断已有文件；先集体设置文件长度。
        MPI_File file;
        int error = MPI_File_open(grid.topology.comm_cart, path.c_str(),
                                  MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &file);
        if (error != MPI_SUCCESS) throw std::runtime_error("Cannot open output: " + path);
        int resized = MPI_File_set_size(file, static_cast<MPI_Offset>(width) * height);
        MPI_File_close(&file);
        if (resized != MPI_SUCCESS) throw std::runtime_error("Cannot resize output: " + path);
...
Grid initial_grid(int w, int h, const std::string& pattern, unsigned seed) {
    ...
    if (pattern == "random") {
        ...
        for (auto& cell : grid) cell = rng() % 100 < 25;
    } else if (pattern == "glider") {
        int x = w / 2 - 1, y = h / 2 - 1;
        grid[y * w + x + 1] = 1;
        grid[(y + 1) * w + x + 2] = 1;
        for (int dx = 0; dx < 3; ++dx) grid[(y + 2) * w + x + dx] = 1;
    } else throw std::runtime_error("Pattern must be random or glider");
    return grid;
}
...
void self_test() {
    auto compare = [](Grid grid, int w, int h, int steps) {
        ...
        for (int i = 0; i < steps; ++i) {
            ...
            gpu.step();
            if (gpu.read() != grid) throw std::runtime_error("CPU/GPU mismatch at step " + std::to_string(i + 1));
        }
    };
    Grid block(64, 0);
    ...
    still.step();
    if (still.read() != block) throw std::runtime_error("Block should remain stable");
    ...
    oscillator.step();
    if (oscillator.read() != vertical) throw std::runtime_error("Blinker first phase failed");
    oscillator.step();
    if (oscillator.read() != blinker) throw std::runtime_error("Blinker period failed");
    ...
    if (moving.read() != translated) throw std::runtime_error("Glider translation failed");
    ...
    if (world_rank == 0) std::cout << "PASS: block, blinker, glider, wraparound, empty/full and random CPU/GPU checks\n";
}
...
int run(int argc, char** argv) {
    try {
        int w = 60, h = 25, steps = 200, delay = 80, seed = 42;
        bool headless = false, test = false;
        std::string pattern = "glider", output;
        for (int i = 1; i < argc; ++i) {
            std::string arg(argv[i]);
            if (arg == "--help") {
                if (world_rank == 0) std::cout << "MPI + CUDA Conway's Game of Life (toroidal boundaries, B3/S23)\n"
            ...
            if (arg == "--width") w = number(value, 5, 8192);
            else if (arg == "--height") h = number(value, 5, 8192);
            else if (arg == "--steps") steps = number(value, 0, 1000000);
            else if (arg == "--delay-ms") delay = number(value, 0, 10000);
            else if (arg == "--seed") seed = number(value, 0, 2147483647);
        ...
        auto render = [&](int generation) {
            ...
            std::cout << "\033[HGeneration " << generation << " / " << steps << "   \n";
            for (int y = 0; y < h; ++y) {
                for (int x = 0; x < w; ++x) std::cout << (grid[y * w + x] ? '#' : '.');
                std::cout << '\n';
            }
            std::cout << std::flush;
        };
        if (!headless) { if (world_rank == 0) std::cout << "\033[2J"; render(0); }
        ...
        grid = sim.read();
        if (!output.empty()) sim.write(output);
        if (world_rank == 0) std::cout << "Grid: " << w << 'x' << h << ", generations: " << steps
...
int main(int argc, char** argv) {
    ...
    try {
        bool help = false;
        for (int i = 1; i < argc; ++i) if (std::string(argv[i]) == "--help") help = true;
        if (!help) {
            int devices = 0, local_rank = 0;
            check(cudaGetDeviceCount(&devices));
            if (!devices) throw std::runtime_error("No CUDA devices available");
            MPI_Comm local;
